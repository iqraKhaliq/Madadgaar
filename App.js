...
// {state.isLoading ? (
//   <StackMain.Screen name="uploadScreen" component={UploadScreen} options={{headerShown: false}}/>
...
const [state,dispatch]= React.useReducer(
    (prevState, action) =>{
      switch(action.type)
      {
        ...
        case 'SIGN_IN':
          // if(action.token)
          // {
          //   AsyncStorage.getItem('userToken',action.token);
          // }
          return{
            ...prevState,
            isSignOut: false,
  ...
  const authContext= React.useMemo(() => ({
    signIn: async data => {
      ...
      dispatch({type: 'SIGN-IN',token: 'auth-token'});
    },
    ...
    signUp: async data => {
      ...
      //                             .getIdToken()
      //                             .then((idToken) => {
      //                               dispatch({type:'SignIn',token: idToken});
      //                             })
      //                             .catch((e) => {
      //                               ToastAndroid.show(e.toString(),ToastAndroid.SHORT,ToastAndroid.BOTTOM);
      //                             });
